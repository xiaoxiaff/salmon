...
bool buildPerfectHash(const std::string& outputDir, std::string& concatText,
                      ...
                      uint32_t numHashThreads, std::shared_ptr<spdlog::logger> duckLog) {
  ...
  IndexT kmerTry = 0, kmerCollision = 0;
  ...
  duckLog->info("perfect hash finished, kmer: {0}, totaltry: {1}, unique kmer: {2}, invalid time: {3}", k, stop, kmerTry, kmerCollision);
...
bool buildHash(const std::string& outputDir, std::string& concatText,
               size_t tlen, uint32_t k, std::vector<IndexT>& SA, std::shared_ptr<spdlog::logger> duckLog) {
  ...
  duckLog->info("hash finished, kmer: {0}, totaltry: {1}, unique kmer: {2}, invalid time: {3}, khash size: {4}", 
               k, stop, kmerTry, kmerCollision, khash.size());
...
void indexTranscriptsSA(ParserT* parser,
                        ...
                        std::shared_ptr<spdlog::logger> duckLog) {
  ...
  duckLog->info("concatText size: {0}", tlen);
  ...
  if (largeIndex) {
    ...
    if (usePerfectHash) {
      duckLog->info("use pefect hash, k is used");
      success = buildPerfectHash<IndexT>(outputDir, concatText, tlen, k, SA,
                                         numHashThreads, duckLog);
    } else {
      duckLog->info("use no pefect hash, k is used");
      success = buildHash<IndexT>(outputDir, concatText, tlen, k, SA, duckLog);
    }
    if (!success) {
...
int rapMapSAIndex(int argc, char* argv[]) {
  ...
  */
  ...
  std::string duckLogPath = indexDir + "duck.log";
  auto fileSink = std::make_shared<spdlog::sinks::simple_file_sink_st>(logPath);
  auto duckFileSink = std::make_shared<spdlog::sinks::simple_file_sink_st>(duckLogPath);
  auto consoleSink = std::make_shared<spdlog::sinks::stderr_sink_st>();
  auto consoleLog = spdlog::create("stderrLog", {consoleSink});
  auto fileLog = spdlog::create("fileLog", {fileSink});
  auto jointLog = spdlog::create("jointLog", {fileSink, consoleSink});
  auto duckLog = spdlog::create("duckLog", {duckFileSink});
  ...
  indexTranscriptsSA(transcriptParserPtr.get(), indexDir, noClipPolyA,
                     usePerfectHash, numPerfectHashThreads, sepStr, iomutex, jointLog, duckLog);
